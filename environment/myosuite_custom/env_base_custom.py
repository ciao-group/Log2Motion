...
class MujocoEnvCustom(gym.Env, gym.utils.EzPickle, ObsVecDict):
    """
    ...
    def __init__(self,  model_path, obsd_model_path=None, seed=None, env_credits=DEFAULT_CREDIT):
        ...
        # sims
        self.sim = SimSceneCustom.get_sim(model_path)
        self.sim_obsd = SimSceneCustom.get_sim(obsd_model_path) if obsd_model_path else self.sim
        self.sim.forward()
    ...
    def _forward(self, **kwargs):
        ...
        if self.mujoco_render_frames:
            self.mj_render()
        self.sim.renderer.refresh_window()
        # observation
